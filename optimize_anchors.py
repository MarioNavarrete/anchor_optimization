...
def generate_config_file(ratios,scales,path):
  try:
  ...
  config.add_section('anchor_parameters')
  config.set('anchor_parameters', 'sizes', '32 64 128 256 512')
  config.set('anchor_parameters', 'strides', '8 16 32 64 128')
  config.set('anchor_parameters', 'ratios', ratios)
  config.set('anchor_parameters',  'scales', scales)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--output',type=str, help='output path of config file', default='/')
    ...
    print(f'Number of labels that don\'t have any matching anchor: {not_matched}')
    path = args.output
    ratios = sorted(np.round(anchor_params.ratios, 3))
    scales = sorted(np.round(anchor_params.scales, 3))

    r = str(ratios)[1:-1].replace(',','')
    s = str(scales)[1:-1].replace(',','')
    generate_config_file(r,s,path)